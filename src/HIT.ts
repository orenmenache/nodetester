    ...
    categories: {
        async ALL(DB: MYSQL_DB) {
            ...
            for (const sport of sports) {
                if (sport.name !== 'Baseball') continue;
                    ...
                    for (const category of categories) {
                        ...
                        for (const group of groups) {
                            ...
                            for (const tournament of tournaments) {
                                ...
                                // console.log(
                                //     `TIMES: ${JSON.stringify(times, null, 4)}`
                                // );

                                const startDateMilli =
                                    Number(times.startDateTimestamp) * 1000;

                                const endDateMilli =
                                    Number(times.endDateTimestamp) * 1000;

                                console.log(
                                    `%cstartDateMilli: ${startDateMilli}`,
                                    'color: orange'
                                );
                                console.log(
                                    `%cendDateMilli: ${endDateMilli}`,
                                    'color: orange'
                                );
                                ...
                                const startDateTimestamp =
                                    formatDateToSQLTimestamp(startDate);

                                if (startDateTimestamp === null) continue;

                                console.log(
                                    `%cstartDateTimestamp: ${startDateTimestamp}`,
                                    'color: green'
                                );

                                const endDateTimestamp =
                                    formatDateToSQLTimestamp(endDate);

                                if (endDateTimestamp === null) continue;

                                console.log(
                                    `%cendDateTimestamp: ${endDateTimestamp}`,
                                    'color: green'
                                );

                                if (endDate.getTime() < nowTimestamp) {
                                    console.warn(
                                        `Tournament has already ended: ${tournament.id} ${tournament.name}`
                                    );
                                    continue;
                                }
                        ...
                        try {
                            if (dbTournaments.length === 0) continue;
                            ...
                            console.warn(
                                `Failed to insert tournaments for category ${category.id} ${category.name}: ${e}`
                            );
                            continue;
                        }
                    }
            ...
            for (const sport of sports) {
                ...
                try {
                    ...
                    */
                    ...
                    for (const tournament of tournaments) {
                        try {
                            ...
                            if (filtered.length === 0) {
                                ...
                                console.log(
                                    `Let's see what we're missing: ${JSON.stringify(
                                        leagueSeasons,
                                        null,
                                        4
                                    )}`
                                ...
                                filtered.map(
                                    ({ id, name, year }: ASA.LeagueSeason) => {
                                        return {
                                            ...
                                            year,
                                            has_last_matches: false,
                                            has_last_matches_within_last_month:
                                                false,
                                            has_next_matches: false,
                                            has_standings: false,
                    ...
                    console.log(`%c${sport.name} done`, 'color: green');
