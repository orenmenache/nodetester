 ...
 */
export async function getNextMatches__GENERIC(
    sportName: DB.SportName,
    DB: MYSQL_DB
) {
    // : Promise<boolean> {
    const funcName = `getNextMatches__GENERIC`;
    ...
    const templateUrl = `https://allsportsapi2.p.rapidapi.com/api/${hyphenated}/tournament/tournamentId/season/seasonId/matches/next/pageNum`;
        ...
        for (const ls of leagueSeasons) {
            for (let pageNum = 0; pageNum < 12; pageNum++) {
                try {
                    ...
                    // console.log(`first: ${JSON.stringify(first, null, 4)}`);
                    // return;
                    const dbNextMatches: DB.AmericanFootball.LastMatch[] =
