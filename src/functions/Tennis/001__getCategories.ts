 ...
 * Tested 201123
 */
export async function getCategories__TENNIS(DB: MYSQL_DB) {
    const funcName = `getCategories__TENNIS`;
    try {
        // await DB.cleanTable(TABLES.TennisCategories.name);

        const url = allSportsAPIURLs.TENNIS.categories;
