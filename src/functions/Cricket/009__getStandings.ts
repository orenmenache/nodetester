...
export default async function getStandings__CRICKET(DB: MYSQL_DB) {
    try {
        await DB.cleanTable(CRICKET.standings);
        ...
        for (const ls of leagueSeasons) {
            const axiosRequest = buildGetRequest(
                allSportsAPIURLs.CRICKET.standings,
                {
            ...
            const response: ASA.Cricket.Responses.Standings =
            ...
            for (const row of response.data.standings[0].rows) {
                ...
                const TeamExists = await DB.SELECT<DB.Team>(CRICKET.teams, {
                    id: row.team.id,
                });
            ...
            await DB.INSERT_BATCH<DB.Cricket.Standings>(
                standingsDB,
                CRICKET.standings,
                false
            );
        ...
        console.warn(`Error in getStandings__CRICKET: ${e}`);
    }
}
