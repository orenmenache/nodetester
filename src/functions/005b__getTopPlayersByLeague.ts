...
import { AllSports__Statistics, AllSports__TopPlayerBundle, AllSports__TopPlayerCategories, AllSports__TopPlayersAPIResponse, DB__Statistics } from '../types/allSportsApi/TopPlayers';
dotenv.config();

export async function getTopPlayersByLeague(DB: MYSQL_DB) {
    const funcName = `getTopPlayersByLeague`;
    try {
        ...
        for (const ls of leagueSeasons) {
            try {
                const url = allSportsAPIURLs.topPlayers
